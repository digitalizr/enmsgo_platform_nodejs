...
const registerCompany = async (req, res) => {
  ...
  if (!name || !company_type || !lead_name || !email || !phone) {
    return res.status(400).json({ message: "All fields are required" });
  }

  try {
    const query = `
      INSERT INTO companies (name, company_type, lead_name, email, phone, created_at)
      VALUES ($1, $2, $3, $4, $5, NOW())
      RETURNING *;
    `;
    const values = [name, company_type, lead_name, email, phone];
    const result = await client.query(query, values);

    res
      .status(201)
      .json({ message: "Company registered", company: result.rows[0] });
  } catch (error) {
